"""
SCA Screening - Deployment: Cost-Aware Configuration K=5
================================================================
Soft-voting ensemble trained on the top-5 SHAP-ranked MoRIs (cost-aware reduced configuration).
"""
...
# Load K-MoRI dataset generated by selecting top-K columns from mori_dataset_K20.xlsx
# ── User Configuration ──────────────────────────────────────────────────────
# This script uses the top-5 SHAP-ranked MoRIs (K=5 deployment configuration).
# DATA_PATH: K=5 MoRI dataset generated by selecting the top-5 MoRI columns from
#            outputs/mori/mori_dataset_K20.xlsx, then saving as mori_dataset_K5.xlsx.
# OUTPUT_DIR: directory where results will be saved.
DATA_PATH = "outputs/mori/mori_dataset_K5.xlsx"
OUTPUT_DIR = "outputs/deployment/K5"
# ────────────────────────────────────────────────────────────────────────────
...
def load_data():
    """Load the K-MoRI dataset from file."""
    logger.info(f"Loading module risk data: {DATA_PATH}")
    df = pd.read_excel(DATA_PATH)
    
    # Identify MoRI score columns (names containing "risk" or "score")
...
def get_stable_models():
    """Return the four base learners for the soft-voting ensemble."""
    return {
...
class EnsemblePredictor:
    """Wrapper exposing predict_proba for KernelExplainer."""
...
def calculate_auc_ci(auc_values, confidence=0.95):
    """
    Compute mean AUC and 95% CI across CV folds.
...
def run_stable_cv(X, y, n_folds=10):
    """Run stratified 10-fold cross-validation for the K-MoRI ensemble."""
...
def compute_kernel_shap(X, y, all_models, all_scalers, n_background=100):
    """
    Compute Kernel SHAP values for the K-MoRI ensemble.

    Args:
        X: K-MoRI feature matrix.
        y: Binary target vector.
        all_models: Per-fold trained model dicts.
        all_scalers: Per-fold scalers.
    ...
    if len(X) > n_background:
        ...
        # Stratified sampling to balance classes in background set
    ...
    # Stratified selection to cover both classes
    ...
    # KernelExplainer returns [neg_class, pos_class] for binary; take index 1
...
def create_shap_dataframe(shap_values, X_explain):
    """Assemble a SHAP statistics DataFrame for Excel output."""
    # Mean |SHAP| per feature
    mean_abs_shap = np.abs(shap_values).mean(axis=0)
...
def create_final_metrics_table(results):
    """
    Assemble the final performance metrics table.
    """
    # AUC 95% CI
...
def main():
    start = datetime.now()
    logger.info("=" * 80)
    logger.info("SCA Screening - Deployment K=5: top-5 SHAP-ranked MoRIs")
    logger.info("=" * 80)
